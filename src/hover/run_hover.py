...
from transformers import (BERT_PRETRAINED_CONFIG_ARCHIVE_MAP, WEIGHTS_NAME,
                          AdamW, BertConfig, BertTokenizer,
...
def evaluate(args, model, tokenizer, global_step, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            #     result = DefactoResult(unique_id, start_logits, end_logits)
            logits = output[0]
    ...
    if args.eval_on_train_data:
        output_prediction_file = os.path.join(
            output_dir, "train_predictions_{}.json".format(prefix)
        )
    else:
        output_prediction_file = os.path.join(
            output_dir, "dev_predictions_{}.json".format(prefix)
        )
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    if args.sub_task == "doc_retrieval":
        from .my_transformers.data.processors.hover_doc import (
            HoverV1Processor, hover_convert_examples_to_features)
    elif args.sub_task == "sent_retrieval":
        from .my_transformers.data.processors.hover_sent import (
            HoverV1Processor, hover_convert_examples_to_features)
    elif args.sub_task == "claim_verification":
        from .my_transformers.data.processors.hover_verif import (
            HoverV1Processor, hover_convert_examples_to_features)
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}_{}".format(
            ...
            (
                "oracle"
                if args.oracle and (not evaluate or args.eval_on_oracle)
                else "normal"
            ),
        ),
    )
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and (
            ...
            examples = HoverV1Processor().get_examples_from_dataset(
...
def main():
    ...
    parser.add_argument("--oracle", action="store_true", help="")
    parser.add_argument("--sub_task", type=str, default="doc_retrieval", help="")
    parser.add_argument("--eval_on_train_data", action="store_true", help="")
    parser.add_argument("--eval_on_oracle", action="store_true", help="")
    parser.add_argument("--ckpt_to_evaluate", type=str, default="100")
    ...
    with ProcessMonitor(dataset=args.dataset_name) as pm:
        ...
        if args.sub_task == "doc_retrieval":
            from .my_transformers.modeling_bert import \
                BertForMultiClassMultipleChoice
            ...
            from .my_transformers.modeling_bert import \
                BertForMultiClassMultipleChoiceSp
        ...
        args.output_dir = os.path.join(
            "out", args.dataset_name, args.output_dir, args.sub_task
        )
        ...
        if args.do_eval and args.local_rank in [-1, 0]:
            ...
            checkpoints = [
                os.path.join(args.output_dir, "checkpoint-" + args.ckpt_to_evaluate)
            ]
