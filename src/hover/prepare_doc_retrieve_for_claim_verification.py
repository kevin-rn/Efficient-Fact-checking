...
def main():
    ...
    parser.add_argument(
        "--top_k", default=5, type=int, help="Top-k sentences to retrieve for reranking"
    )
        ...
        doc_data = json.load(
            open(
                os.path.join(
                    args.data_dir,
                    "doc_retrieval",
                    "hover_" + args.data_split + "_doc_retrieval.json",
                )
            )
        )
            ...
            if uid in uid_to_doc:
                ...
                if args.rerank_mode == "none":
                    ...
                    context = [
                        sent
                        for _, sent in sorted(
                            sorted_sents[: args.top_k], key=lambda x: x[0]
                        )
                    ]
                    context = " ".join(context)
