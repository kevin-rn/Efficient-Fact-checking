...
def hover_convert_example_to_features(
    example, max_seq_length, max_doc_num, doc_stride, max_query_length, is_training
):
    features = []
    all_docs_tokens = []
...
class HoverProcessor(DataProcessor):
    ...
    def _create_examples(self, input_data, set_type):
        ...
        for entry in tqdm(input_data):
            ...
            labels_temp = entry["labels"]
            titles, paras, labels = [], [], []
            for idx, doc in enumerate(context):
                if doc and len(doc) > 1:
                    titles.append(doc[0])
                    # In our modified version sentences are already split
                    paragraph = " ".join(doc[1:])
                    paras.append(paragraph)
                    labels.append(labels_temp[idx])
