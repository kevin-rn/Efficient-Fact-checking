...
from .nn_utils import BatchNorm, Feedforward, Lambda
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not share_codewords:
            decoder.add_module(
                "embed", nn.Linear(num_codebooks * codebook_size, bottleneck_dim)
            )
        else:
            decoder.add_module(
                "embed",
                Lambda(
                    lambda x: x
                    @ self.codebook.to(device=x.device).view(
                        num_codebooks * codebook_size, bottleneck_dim
                    )
                ),
            )
        ...
        decoder.add_module(
            "ffn",
            Feedforward(
                bottleneck_dim, hidden_dim, num_layers=decoder_layers, **kwargs
            ),
        )
