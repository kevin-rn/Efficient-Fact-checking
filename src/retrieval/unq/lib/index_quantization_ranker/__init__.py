...
from ..knn import FAISSFlatIndex, KNNBase, ReorderSearch
from ..nn_utils import DISTANCES, CallMethod
from ..quantizer import NeuralQuantization, QuantizationBase
from ..utils import check_numpy, process_in_chunks
...
class NeuralQuantizationRanker(IndexQuantizationRankerBase, KNNBase):
    def __init__(
        ...
        **kwargs
        ...
        if quantizer.training:
            warn(
                "quantizer was asked to process base in training mode (with dropout/bn)"
            )

        if self.device_ids is None:
            base_to_codes = quantizer.get_codes
        else:
            base_to_codes = nn.DataParallel(
                CallMethod(quantizer, "get_codes"), device_ids=device_ids, **kwargs
            )

        with torch.no_grad():
            base_codes = process_in_chunks(
                base_to_codes, base, batch_size=batch_size or len(base)
            )
    ...
    def search(self, query, k=1, batch_size=None):
        ...
        if self.device_ids is None:
            ...
            query_to_distances = nn.DataParallel(
                CallMethod(self.quantizer, "get_distances"),
    ...
    def __init__(
        ...
        **kwargs
        ...
        apply_model = (
            ...
            else nn.DataParallel(model, device_ids, **kwargs)
        )
        with torch.no_grad():
            if rerank_k != float("inf"):
                ...
                if rerank_k > 1:
                    ...
                    reconstructed_base = process_in_chunks(
                        apply_model, base, batch_size=batch_size
                    )
                    self.knn = ReorderSearch(
                        reconstructed_base,
                        baseline_search=self.knn,
                        distance=reconstructed_distance,
                        baseline_k=rerank_k,
                        batch_size=reorder_batch_size or batch_size,
                        return_numpy=True,
                    )
                ...
                if reconstructed_distance not in (
                    DISTANCES["euclidian_squared"],
                    DISTANCES["euclidian"],
                ):
                    raise NotImplementedError(
                        "Fast full reconstruction search is only implemented "
