...
from .nn_utils import GumbelSoftmax, Lambda
...
class NeuralQuantization(QuantizationBase):
    def __init__(
        ...
        initial_entropy=1.0,
        key_dim=None,
        decouple_temperatures=True,
        encoder=None,
        decoder=None,
        init_codes_with_data=True,
        **kwargs
    ):
        """
        ...
        self.log_temperatures = nn.Parameter(
            data=torch.zeros(num_codebooks) * float("nan"), requires_grad=True
        )
        self.initial_entropy, self.init_codes_with_data = (
            initial_entropy,
            init_codes_with_data,
        )
    ...
    def compute_logits(self, x, add_temperatures=True):
        ...
        if len(x.shape) > 2:
            flat_logits = self.compute_logits(
                x.view(-1, x.shape[-1]), add_temperatures=add_temperatures
            )
            return flat_logits.view(
                *x.shape[:-1], self.num_codebooks, self.codebook_size
            )
    ...
    def initialize(self, x):
        ...
        with torch.no_grad():
            if self.init_codes_with_data:
                chosen_ix = torch.randint(
                    0,
                    x.shape[0],
                    size=[self.codebook_size * self.num_codebooks],
                    device=x.device,
                )
                chunk_ix = (
                    torch.arange(
                        self.codebook_size * self.num_codebooks, device=x.device
                    )
                    // self.codebook_size
                )
                initial_keys = (
                    self.encoder(x)[chosen_ix, chunk_ix]
                    .view(*self.codebook.shape)
                    .contiguous()
                )
                self.codebook.data[:] = initial_keys

            base_logits = self.compute_logits(x, add_temperatures=False).view(
            ...
            log_temperatures = torch.tensor(
                [
                    fit_log_temperature(
                        codebook_logits,
                        target_entropy=self.initial_entropy,
                        tolerance=1e-2,
                    )
...
def fit_log_temperature(
    logits,
    target_entropy=1.0,
    tolerance=1e-6,
    max_steps=100,
    lower_bound=math.log(1e-9),
    upper_bound=math.log(1e9),
):
    """
...
def compute_penalties(
    logits,
    individual_entropy_coeff=0.0,
    allowed_entropy=0.0,
    global_entropy_coeff=0.0,
    cv_coeff=0.0,
    square_cv=True,
    eps=1e-9,
):
    ...
    if individual_entropy_coeff != 0:
        ...
        clipped_entropy = F.relu(
            allowed_entropy - individual_entropy_values + eps
        ).mean()
        individual_entropy = (
            individual_entropy_values.mean() - clipped_entropy
        ).detach() + clipped_entropy
    ...
    if global_entropy_coeff != 0:
        ...
        global_logp = torch.logsumexp(logp, dim=0) - np.log(
            float(logp.shape[0])
        )  # [..., codebook_size]
