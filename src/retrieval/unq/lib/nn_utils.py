...
class OneCycleSchedule:
    ...
    def step(self, **kwargs):
        self.current_lr = self.update_learning_rate(
            t=self.step_count, **self.learning_rate_opts
        )
    ...
    def load_state_dict(self, state_dict, load_step=True, load_opts=True, **kwargs):
        self.learning_rate_opts = (
            state_dict["learning_rate_opts"] if load_opts else self.learning_rate_opts
        )
...
def get_learning_rate(optimizer):
    ...
    raise ValueError(
        "Could not infer learning rate from optimizer {}".format(optimizer)
    )
