...
from .utils import check_numpy, free_memory, process_in_chunks
...
class FAISSFlatIndex(KNNBase):
    def __init__(
        self, base, use_cuda=torch.cuda.is_available(), device_id=0, temp_memory=True
    ):
        """
        ...
        if use_cuda:
            ...
            self.index_flat = faiss.index_cpu_to_gpu(
                self.res, device_id, self.index_flat
            )
...
class ReorderSearch:
    ...
    def _search(self, query, candidate_ids, k):
        ...
        )  # [num_queries, base_size]
        chosen_in_candidates = torch.topk(distances, k=k, dim=1, largest=False)[
            1
        ]  # [num_queries, k]
    ...
    def search(self, query, k=1, *, baseline_k=None, batch_size=None, **kwargs):
        ...
        nearest_ids = process_in_chunks(
            partial(self._search, k=k), query, candidate_ids, batch_size=batch_size
        )
