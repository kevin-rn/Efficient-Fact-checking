...
from .nn_utils import (DISTANCES, OneCycleSchedule, gumbel_softmax,
                       training_mode)
...
class Trainer(nn.Module):
    def __init__(
        ...
        *,
        ...
        loss_opts=None,
        optimizer=None,
        LearnedSimilaritySearch,
        SimilaritySearch=FAISSFlatIndex,
        NegativeSimilaritySearch=None,
        warm_start=False,
        verbose=False,
        max_norm=None,
        drop_large_grads=False,
        device_ids=None,
        output_device=None,
        batch_dim=0,
        **kwargs
        ...
        if device_ids is not None:
            self.loss = nn.DataParallel(
                self.loss, device_ids, output_device=output_device, dim=batch_dim
            )
        self.opt = optimizer or OneCycleSchedule(
            torch.optim.Adam(model.parameters(), amsgrad=True), **kwargs
        )
        self.NegativeSimilaritySearch = (
            NegativeSimilaritySearch or LearnedSimilaritySearch
        )
        ...
        if experiment_name is None:
            experiment_name = "untitled_{}.{:0>2d}.{:0>2d}_{:0>2d}:{:0>2d}".format(
                *time.gmtime()[:5]
            )
            if self.verbose:
            ...
            if self.drop_large_grads and metrics["grad_norm"] > self.max_norm:
                ...
                for metric in metrics:
                    self.writer.add_scalar(
                        prefix + metric, metrics[metric].mean().item(), self.step
                    )
                return metrics
    ...
    def evaluate_recall(self, base, query, k=1, prefix="dev/", **kwargs):
        ...
        with torch.no_grad(), training_mode(self.model, is_train=False):
            ...
            predicted_indices = self.LearnedSimilaritySearch(base, **kwargs).search(
                query, k=k
            )
            predicted_indices, reference_indices = map(
                check_numpy, (predicted_indices, reference_indices)
            )
    ...
    def evaluate_recall_enwiki(
        self, base, query, reference_indices, k=1, prefix="dev/", **kwargs
    ):
        """Computes average recall @ k"""
        with torch.no_grad(), training_mode(self.model, is_train=False):
            predicted_indices = self.LearnedSimilaritySearch(base, **kwargs).search(
            ...
            recalls = [
                self.calculate_recall(pred, ref[ref != -1], k)
                for pred, ref in zip(predicted_indices, reference_indices)
            ]
            average_recall = sum(recalls) / len(recalls) if len(recalls) > 0 else 0
        self.writer.add_scalar(
            "{}recall@{}".format(prefix, k), average_recall, self.step
        )
    ...
    def load_checkpoint(self, tag=None, path=None, **kwargs):
        ...
        if tag is None and path is None:
            path = get_latest_file(
                os.path.join(self.experiment_path, "checkpoint_*.pth")
            )
    ...
    def get_true_nearest_ids(self, base, *, k, exclude_self=True):
        ...
        with torch.no_grad():
            train_neighbors_index = self.SimilaritySearch(base).search(
                base, k=k + int(exclude_self)
            )[:, int(exclude_self) :]
            original_neighbors_index = torch.as_tensor(
                train_neighbors_index, device=base.device
            )
        if self.verbose:
            print(end="Done\n", flush=True)
    ...
    def get_negative_ids(self, base, positive_ids, *, k, skip_k=0):
        ...
        with torch.no_grad():
            with training_mode(self.model, is_train=False):
                learned_nearest_ids = self.NegativeSimilaritySearch(base).search(
                    base, k=k_total
                )
                learned_nearest_ids = torch.as_tensor(
                    learned_nearest_ids, device=base.device
                )
            # ^-- [base_size, k_total]

            idendity_ids = torch.arange(len(positive_ids), device=positive_ids.device)[
                :, None
            ]  # [batch_size, 1]
            ...
            negative_mask = (
                learned_nearest_ids[..., None] != forbidden_ids[..., None, :]
            ).all(-1)
            # ^-- [base_size, k_total]
            ...
            slices = torch.cat(
                [
                    torch.zeros_like(negative_ii[:1]),
            ...
            squashed_negative_jj = (
                torch.arange(len(negative_jj), device=negative_jj.device)
                - slices[negative_ii]
            )
            ...
            negative_ids = torch.sparse_coo_tensor(
                new_negative_ix, negative_values, size=learned_nearest_ids.shape
            ).to_dense()[:, skip_k : k + skip_k]
            # ^--[base_size, k + skip_k]
    ...
    def get_negative_ids_enwiki(self, base, positive_ids, *, k, skip_k=0):
        ...
        negative_ids = [
            random.sample(list(indices - set(p_id.tolist())), k)
            for p_id in positive_ids
        ]
...
class AutoencoderLoss(LossBase):
    @staticmethod
    def compute_loss(
        model: UNQModel, x_batch, *, distance=DISTANCES["euclidian_squared"], **kwargs
    ):
        """A simple loss that minimizes the :distance: between input vector and model output"""
        x_reconstructed, activations = model.forward(
            x_batch, return_intermediate_values=True
        )
...
class TripletLoss(LossBase):
    ...
    def compute_loss(
        ...
        *,
        hard_codes=True,
        reconstruction_coeff=0.0,
        reconstruction_distance=DISTANCES["euclidian_squared"],
        triplet_coeff=0.0,
        triplet_delta=0.0,
        eps=1e-6,
        **kwargs
        ...
        assert (
            x_positives is not None and x_negatives is not None
        ) or triplet_coeff == 0
        ...
        if triplet_coeff != 0:
            ...
            pos_codes = gumbel_softmax(
                model.compute_logits(x_positives), noise=0.0, hard=hard_codes, dim=-1
            )
            pos_distances = (pos_codes * distances_to_codes).sum(dim=[-1, -2])

            neg_codes = gumbel_softmax(
                model.compute_logits(x_negatives), noise=0.0, hard=hard_codes, dim=-1
            )
