...
def format_for_claim_verification(results, queries, corpus, claim_json, data_split) -> None:
    ...
    retrieve_file = os.path.join("data", args.dataset_name, "claim_verification", 
...
def format_for_sent_retrieval(results, queries, corpus, claim_json, data_split) -> None:
    ...
    for query_id, docs_data in results.items():
        ...
        claim_results.append(json_result)
    conn.close()
    retrieve_file = os.path.join("data", args.dataset_name, "sent_retrieval", 
                        f"hover_{data_split}_sent_retrieval.json")
    with open(retrieve_file, 'w', encoding="utf-8") as f:
        json.dump(claim_results, f)


def search_and_generate_results(model, corpus, queries, claim_json, batch_size, data_split):
    ...
    index_path = os.path.join("data", "indices", "jpq",
    ...
    with ProcessMonitor(dataset=args.dataset_name) as pm:
        ...
        results = dr_jpq.search(corpus=corpus, queries=queries, 
                                top_k=args.top_k_nn, score_function="dot")
        if args.sent_select:
...
def main():
    ...
    jpq_path = (f"models/{setting}/m{args.subvectors_num}/doc_encoder", 
    ...
    if args.data_split:
        ...
        search_and_generate_results(model, corpus, queries, claim_json, args.batch_size, args.data_split)
    else:
