...
from .dataset import SequenceDataset, TextTokenIdsCache, pack_tensor_2D
...
def compute_loss(
    query_embeddings, pq_codes, centroids, batch_neighbors, all_rel_pids, lambda_cut
):
    loss = 0
    mrr = 0
    ...
    for qembedding, retrieve_pids, cur_rel_pids in zip(
        ...
        position_matrix = (1 + torch.arange(len(target_labels))).to(
            target_labels.device
        )
        ...
        pair_weights = torch.abs(
            irrel_weights.reshape(1, -1) - rel_weights.reshape(-1, 1)
        )
...
def train(args, model, pq_codes, centroid_embeds, opq_transform, opq_index):
    ...
    optimizer_grouped_parameters = [
        ...
        },
        {
            ...
            "weight_decay": args.centroid_weight_decay,
            "lr": args.centroid_lr,
        },
    ]
        ...
        for step, (batch, all_rel_poffsets) in enumerate(epoch_iterator):
            ...
            if args.gpu_search:
                batch_neighbors = gpu_index.search(
                    query_embeddings.detach().cpu().numpy(), args.loss_neg_topK
                )[1]
            else:
    ...
    # return the filename of the last FAISS index, and its actual instance
