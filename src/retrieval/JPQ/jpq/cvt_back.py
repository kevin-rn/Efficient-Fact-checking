...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", type=str, choices=["train", "dev", "test", "lead"], required=True
    )
    parser.add_argument("--output_path", type=str, required=True)
    parser.add_argument(
        "--dataset", type=str, choices=["passage", "doc"], required=True
    )
    args = parser.parse_args()

    pid2offset = pickle.load(
        open(os.path.join(args.preprocess_dir, "pid2offset.pickle"), "rb")
    )
    offset2pid = {v: k for k, v in pid2offset.items()}
    qid2offset = pickle.load(
        open(os.path.join(args.preprocess_dir, f"{args.mode}-qid2offset.pickle"), "rb")
    )
