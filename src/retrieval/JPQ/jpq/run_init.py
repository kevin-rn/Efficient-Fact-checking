...
import numpy as np
import torch
from jpq.dataset import (SequenceDataset, TextTokenIdsCache,
...
def doc_inference(model, args, embedding_size):
    ...
    doc_dataset = SequenceDataset(
        ids_cache=ids_cache, max_seq_length=args.max_doc_length
    )
    ...
    try:
        prediction(model, doc_collator, args, doc_dataset, doc_memmap)
    except:
...
def main():
    ...
    if not os.path.exists(args.doc_embed_path):
        ...
        config = RobertaConfig.from_pretrained(
            args.model_dir, gradient_checkpointing=False
        )
    ...
    save_index_path = os.path.join(
        args.output_dir, f"OPQ{args.subvector_num},IVF1,PQ{args.subvector_num}x8.index"
    )
    ...
    index = faiss.index_factory(
        dim,
        f"OPQ{args.subvector_num},IVF1,PQ{args.subvector_num}x8",
