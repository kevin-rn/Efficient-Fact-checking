...
class TextTokenIdsCache:
    def __init__(self, data_dir, prefix):
        ...
        try:
            ...
            self.ids_arr = np.memmap(
                f"{data_dir}/memmap/{prefix}.memmap",
                shape=(self.total_number, self.max_seq_len),
                dtype=np.dtype(meta["type"]),
                mode="r",
            )
    ...
    def collate_function(batch):
        ...
        data = {
            "input_ids": pack_tensor_2D(
                input_ids, default=1, dtype=torch.int64, length=None
            ),
            "attention_mask": pack_tensor_2D(
                attention_mask, default=0, dtype=torch.int64, length=None
            ),
        }
