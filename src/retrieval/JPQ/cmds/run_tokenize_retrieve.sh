...
for year in "2020" "2019"  ; do
    ...
    for dataset in "doc" "passage"; do
        for m in 96 64 48 32 24 16; do
            ...
            trec_data_dir="./data/jpq/${dataset}/trec20-test"
            query_file_path="${trec_data_dir}/msmarco-test${year}-queries.tsv "
            pid2offset_path="./data/jpq/${dataset}/preprocess/pid2offset.pickle"
            output_path=./data/jpq/$dataset/tokenize_retrieve/official.run.trec${year}.m${m}.rank
            ...
            if [ $dataset = "passage" ] 
            then
                ./data/jpq/trec_eval-9.0.7/trec_eval -c -mndcg_cut.10 -mrecall.100 $trec_data_dir/${year}qrels-pass.txt $output_path
            else
                ./data/jpq/trec_eval-9.0.7/trec_eval -c -mndcg_cut.10 -mrecall.100 $trec_data_dir/${year}qrels-docs.txt $output_path
            fi
