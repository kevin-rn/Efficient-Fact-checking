...
class ElasticSearch(object):
    ...
    def __init__(self, es_credentials: Dict[str, object]):
        ...
        self.es = Elasticsearch(
            ...
            use_ssl=True,
            verify_certs=True,
            http_auth=("elastic", "EEjHn3N6AZnHPCia-Zfm"),
            ca_certs="/home/venky/numerical-llm/elasticsearch-8.11.3/config/certs/http_ca.crt",
        )
    ...
    def check_language_supported(self):
        ...
        if self.language.lower() not in self.languages:
            raise ValueError(
                "Invalid Language: {}, not supported by Elasticsearch. Languages Supported: \
            {}".format(
    ...
    def get_index_size(self):
        ...
        if index_stats and "indices" in index_stats:
            size_in_bytes = index_stats["indices"][self.index_name]["total"]["store"][
                "size_in_bytes"
            ]
    ...
    def lexical_search(
        self, text: str, top_hits: int, ids: List[str] = None, skip: int = 0
    ) -> Dict[str, object]:
        """[summary]
    ...
    def lexical_multisearch(
        self, texts: List[str], top_hits: int, skip: int = 0
    ) -> Dict[str, object]:
        ...
        assert (
            skip + top_hits <= 10000
        ), "Elastic-Search Window too large, Max-Size = 10000"
