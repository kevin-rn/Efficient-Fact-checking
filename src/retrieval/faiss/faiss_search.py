...
from src.retrieval.unq.lib import Trainer, UNQModel, UNQSearch
from src.tools.monitor_utils import format_size
...
class FaissSearch:
    def __init__(
        ...
        if use_gpu:
            ...
            self.ann = faiss.index_cpu_to_gpu(self.gpu_res, 1, flat_index)
        else:
    ...
    def setup_compression_model(
        self, checkpoint_path: str, vect_dim: int, n_codebooks: int
    ):
        device_ids = [0]
        ...
        trainer = Trainer(
            ...
            loss_opts={},
            LearnedSimilaritySearch=partial(
                UNQSearch,
                model=model,
                rerank_k=500,
                batch_size=1000,
                reorder_batch_size=250,
                device_ids=device_ids,
            ),
            NegativeSimilaritySearch=partial(
                UNQSearch,
                model=model,
                rerank_k=1,
                batch_size=1000,
    ...
    def get_top_n_neighbours(
        self, query_list: List[Any], top_k: int, batched: bool, doc_database: str
    ) -> Dict:
        """
        ...
        if batched:
            with torch.no_grad():
                embeds = self.encoder.encode(
                    query_list, batch_size=128, show_progress_bar=True
                )
                if self.use_compress:
