...
class Extractor(object):
    ...
    def transform(self, wikitext):
        ...
        for m in nowiki.finditer(wikitext, cur):
            ...
            nowiki_chunks[nowiki_key] = wikitext[
                m.start() + len("<nowiki>") : m.end() - len("</nowiki>")
            ]
            nowiki_id += 1
            cur = m.end()
        # leftover
        ...
        # replace wiki chunks with actual nowiki text
    ...
    def expandTemplate(self, body):
        ...
        if colon > 1:
            ...
            if (
                subst
                and self.frame.depth > 0
                and funct == "#invoke"
                and len(parts) <= 2
            ):
                # if substitution happened, the actual arguments are probably on the previous frame
                self.frame.pop()
...
modules = {
    "convert": {
        "convert": lambda params: params["1"] + " " + params["2"],  # no conversion
    },
