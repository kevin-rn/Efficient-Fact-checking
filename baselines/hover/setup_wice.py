...
def convert_wice():
    ...
    for data_split in ["train", "dev", "test"]:
        ...
        if data_split == "test":
            hover_format_claims = [{"uid": claim_obj['meta']['id'], 'claim': claim_obj['claim']} 
            ...
            for claim_obj in claim_json:
                ...
                support_sents = list(dict.fromkeys(chain.from_iterable(claim_obj['supporting_sentences'])))
                supporting_facts = [[article_title, sent_no] for sent_no in support_sents]
                label = "SUPPORTED" if claim_obj['label'] == "supported"else "NOT_SUPPORTED"
                hover_format_claims.append({"uid": uid, 'claim': claim, 'supporting_facts': supporting_facts, 'label': label})
