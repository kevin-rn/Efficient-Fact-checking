...
python3 -m src.retrieval.bm25.run_bm25_search \
  --dataset_name=$claim_name \
  --setting=$setting

echo -e "\nDOC RETRIEVAL"
conda activate hover
python3 -m src.hover.prepare_data_for_doc_retrieval \
  --dataset_name=$claim_name \
  --data_split=train \
...
python3 -m src.hover.prepare_data_for_doc_retrieval \
  ...
  --doc_retrieve_range=20 \
  --setting=$setting
./src/hover/train_scripts/train_doc_retrieval.sh $claim_name
./src/hover/eval_scripts/eval_doc_retrieval_on_train.sh $claim_name
./src/hover/eval_scripts/eval_doc_retrieval_on_dev.sh $claim_name
...
if [ "$bm25_type" = "original" ]
  then
    ...
    python3 -m src.hover.prepare_data_for_sent_retrieval \
      --data_split=train \
    ...
    echo -e "\nCLAIM VERIFICATION"
    conda activate grounding
    python3 -m src.hover.prepare_doc_retrieve_for_claim_verification \
      --dataset_name=$claim_name \
      --data_split=train \
